"""HRV feature extraction — must stay in sync with ml/src/features.py."""
...
def features_to_vector(features: dict[str, float], columns: list[str] | None = None) -> list[float]:
    cols = columns or FEATURE_COLUMNS
    return [float(features.get(col, 0.0)) for col in cols]
